...
<header class="bg-gray-600 lg:py-0 py-2">
    <div class="w-full flex flex-wrap items-center md:w-3/4 px-6 md:px-0 md:mx-auto">
      ...
      <div class="brand">
          <a href="#hero"><h1 style="color: #ooo;"><span>Eastern</span>Cape <span>C</span>abs</h1></a>
        </div>
      </div>
        ...
        <?php
              ...
              //store the ID from the previous page
               ...
               if(isset($_COOKIE["clientsID"]))
               {
                    ...
                    $query = "SELECT profileImage FROM clients WHERE clientsID = $id";
      ...
      if( isset($_COOKIE["clientsID"]))
      {
          ...
          <div class="realtive">
            ...
            <div hidden id="myDIV" class="absolute top-auto  mt-2 py-2 w-48 bg-gray-300 rounded-lg shadow-xl"> 
              ...
              <a href="../profile_page/profile12.php?clientsID='.$clientID.'" class="block px-4 py-2 hover:bg-indigo-500 hover:text-white">Profile</a>
...
<?php 
    ...
    if(isset($_COOKIE["clientsID"]))
    {
        ...
        //connect to the database
        $connection = mysqli_connect("ecabs.mysql.database.azure.com", "admincab@ecabs", "password123!", "ECabs") or die("Could not connect to database!");
        
        //issue the query instructions
        $query = "SELECT lastlame, firstName, email, contact FROM clients WHERE clientsID = $id";
        $result = mysqli_query($connection, $query) or die("Could not retrieve data!");
  ...
  <div class="flex flex-wrap -mx-3 mb-6">
    ...
    <div class="w-full md:w-1/2 px-3">
      <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="grid-last-name" style="padding-top: 0.75rem;padding-bottom: 0.75rem;right: 5px;padding-left: 0.75rem;">
       ...
       <input type="text" id="lastlame" name="lastlame" class="border rounded shadow appearance-none focus:outline-none focus:shadow-outline mb-6" value="<?php echo $lastname;?>" style="padding-top: 0.75rem;padding-bottom: 0.75rem;right: 5px;padding-left: 0.75rem;">
	</div><br>
    <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
      <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" style="padding-top: 0.75rem;padding-bottom: 0.75rem;right: 5px;padding-left: 0.75rem;" for="grid-first-name">
       Email
         ...
         <input type="e_mail" id="email" name="email" class="border rounded shadow appearance-none focus:outline-none focus:shadow-outline" value="<?php echo $email;?>" style="padding-top: 0.75rem;padding-bottom: 0.75rem;right: 5px;padding-left: 0.75rem;">
    </div>
    <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
      <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" style="padding-top: 0.75rem;padding-bottom: 0.75rem;right: 5px;padding-left: 0.75rem;" for="grid-first-name">
       Contact Number
         ...
         <input type="text" id="contact" name="contact" class="border rounded shadow appearance-none focus:outline-none focus:shadow-outline" value="<?php echo $contactnum;?>" style="padding-top: 0.75rem;padding-bottom: 0.75rem;right: 5px;padding-left: 0.75rem;">
    </div>
  </div>
  <br><br>
